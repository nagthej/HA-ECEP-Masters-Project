...
Docker Client python API to create, start, 
...
def create_containers(args):
    ...
    #if '--runtime=nvidia' in args:
        #args['--runtime=nvidia'] = str(args['--runtime=nvidia'])
    ...
    try:
        ...
        command="sudo -S docker run --name "+str(args['name'])+" -t --runtime=nvidia "+str(args['image'])
        print(command)
        os.popen(command, 'w').write("220@CMPE")
