...
class addFile:
    ...
    def transferFile(self, **kwargs):
        ...
        try:
            _fileObj = open(self._localPath, 'rb')
    ...
    def copyFileTo_container(self, **kwargs):
        ...
        _transferApp = {'container_name': self._containerName, \
          ...
          #               'local_path': '/home/Downloads/testing.tar'}
    ...
    def fetch_results_using_cp(self, **kwargs):
        ...
        self.resource = kwargs['path_to_retrieveFile']

        print('container_name:  , container_path:', self.containerName, self.resource)
        #invoke_clientAPI = Client(base_url='tcp://127.0.0.1:2375')
        c = Client(base_url='unix://var/run/docker.sock', version='1.12')
        ...
        try:
            print('#######################33333333try')
            _fileObtained = c.copy(self.containerName, self.resource)
            print('response: ############', _fileObtained)
            filelike = io.BytesIO(_fileObtained.read())  # Sends contains of file as string. Converting it back to a file.
