...
def callContainer(data):
    ...
    if data['command'] == 'upStart':
        ...
        cmd['local_path'] = ecep_endNode.ecep_wampClient.fetcher.get_file(**kwargs)
    ...
    if data['command'] == 'containerinfo':
        cmd['container'] = data['containerName']

        response['success'] = container.info_container(cmd)
    ...
    if data['command'] == 'download':
        fetch_kwargs = {'container': data['containerName'], 'path_to_retrieveFile': data['filename']}
        ...
        response['success'] = ecep_endNode.ecep_wampClient.fetcher.put_file(**put_kwargs)
