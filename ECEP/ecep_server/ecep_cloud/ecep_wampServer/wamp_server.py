...
Implementation of WAMP server subscription API
"""
...
class ClientReader(ApplicationSession):
    ...
    def onJoin(self, details):
        ...
        def deviceResponse(args):
            ...
            log.msg('device Respone received: ', args)
            uDB_instance = uDB()
           # node = Compute_Manager()
            #if node.get_compute_node_list(username='admin') == False:
             #  node.add_compute_node_list(username="admin", containerName="/pedantic_euclid")
        ...
        def containerStatus(args):
            ...
            #uDB_instance.addComputeNode(args)
            #node = Compute_Manager()
            #if node.get_compute_node_list(username='admin') == False:
             #  node.add_compute_node_list(username="admin", containerName="/pedantic_euclid")
            # handle the device response
