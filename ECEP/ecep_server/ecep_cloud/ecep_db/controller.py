...
sqlalchemy DB API implmentation
"""
...
class Device_Manager():
    ...
    def get_device_list(self):
        ...
        try:
            ...
            pass
        db_lock.release()
        print("****************************get_device_list*******************************8888", ret)
        return ret
...
class Compute_Manager():
    ...
    def add_new_compute_node(self, **kwargs):
        ...
        db_lock.acquire()
        node = Compute(**kwargs)
        print ("add_new_compute_node", kwargs)
        try:
            db_session.add(node)
            db_session.commit()
        except Exception as e:
    ...
    def update_compute_node(self, **kwargs):
        ...
        db_lock.acquire()
        try:
            print ("update_compute_node kwargs", kwargs)
    ...
    def get_compute_node_list(self, **kwargs):
        print ("*********************compute get_compute_node_list*************")
        global db_session
        global db_lock
        ...
        try:
            print ("*******************kwargs**********************")
            print (kwargs)
            #list_db = db_session.query(Compute).filter_by(containerName ='test').all()
            ...
            for u in list_db:
                ...
                print("new#######################################")
                db_arr.append(new)
            print ("db_arr*********************compute get_compute_node_list*************")
            #print ("db_arr", db_arr)
            ret = {'compute': db_arr}
        except Exception as e:
            ret = "{error:%s}" % str(e)
            print("#####################get_compute_node_list exception ######################")
            pass
        db_lock.release()
        print ("$$$###################ret", ret)
        return ret
...
if __name__ == '__main__':
    ...
    print(json.dumps(ret))
    node = Compute_Manager()
    ret = node.get_compute_node_list(username='deepa')
    print(json.dumps(ret))
